...
public class JukeboxCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] args) {
        if(!commandSender.hasPermission("jukeboxapi.admin")){
            commandSender.sendMessage(PREFIX + "https://www.mcjukebox.net/?username=" + commandSender.getName() + "&server=" + Jukebox.getInstance().getId());
            return true;
        }
        if(args.length == 3){
            if(args[0].equalsIgnoreCase("music") | args[0].equalsIgnoreCase("sound")){
                ...
                if(playFor == null){
                    commandSender.sendMessage(PREFIX + args[1] + " is not currently online.");
                    return true;
                }
                ...
                commandSender.sendMessage(PREFIX + "Usage: /jukeboxapi <music/sound> <player> <url>");
            }
